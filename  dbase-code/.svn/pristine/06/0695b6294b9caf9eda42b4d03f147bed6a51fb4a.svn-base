...
{
   USES_CONVERSION;
   TCHAR tablename[_MAX_PATH];
   _tsplitpath(filename, NULL, NULL, tablename, NULL);
   std::string fileName = T2CA(filename); 
   std::string tableName = T2CA(tablename); 
   bool ok = base::Open(fileName.c_str(), editmode, tableName.c_str());
...
inline int CDbaseFile::Open(const TCHAR* filename, const DBF_OPEN& parm)
{
   USES_CONVERSION;
   std::string fileName = T2CA(filename); 
   BOOL ok = base::Open(fileName.c_str(), parm);
...
inline bool CDbaseFile::Create(const TCHAR* fileName, 
   void* stream, struct zlib_filefunc_def_s* api, 
   const DBaseFieldVector& vector, 
   dbf_charconv charconv, void* memoStream)
{
    DBaseCreateParm parm;
    parm.api = api;
    parm.vector = vector;
    parm.charconv = charconv;
   bool ok = base::Create(stream, parm, memoStream);
...
inline const CString& CDbaseFile::GetFilename() const
{
...
{
   return dbf_putrecord(GetHandle(), index - 1) ? DBASE_SUCCESS : base::GetLastError();
}
...
{
   return ::dbf_insertrecord(GetHandle(), index) ? DBASE_SUCCESS : base::GetLastError();
}
...
inline DBF_HANDLE CDbaseFile::Detach()
{
   m_strFileName.Empty();
   return base::Detach();
}
...
inline bool CDbaseFile::PutRecord()
{
