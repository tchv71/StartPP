...
wxDataViewRenderer* DataModelListCtrl::GetRenderer(const wxString& datatype) const
{
    if (wxT("bool") == datatype)
        return new MyDataViewToggleRenderer(datatype, wxDATAVIEW_CELL_ACTIVATABLE);
    return new wxDataViewTextRenderer(datatype, wxDATAVIEW_CELL_EDITABLE);
}
...
void DataModelListCtrl::Copy()
{
    const wxDataModelBase* model = GetModel();
    wxArrayInt rows;
    wxArrayString as;

    GetRowSelections(rows);
    for (wxArrayInt::const_iterator it = rows.begin(); it != rows.end(); it++)
    {
        wxArrayString row_str;
        for (size_t col = 0; col < model->GetColumnCount(); col++)
        {
            wxString str;
            model->GetValueByRow(&str, *it, col);
            row_str.push_back(str);
        }
        as.push_back(wxJoin(row_str, ';', 0));
    }
    wxClipboardHelper::SetText(wxJoin(as, '\n', 0));
}

/////////////////////////////////////////////////////////////////////////////
// DataModelListCtrlAdaptor

wxWindow* DataModelListCtrlAdaptor::GetItemsCtrl() const
{
    return m_listCtrl;
}

bool DataModelListCtrlAdaptor::CanAdd() const
{
    return true;
}

bool DataModelListCtrlAdaptor::CanRemove() const
{
    DataModelListCtrlAdaptor* pThis = const_cast<DataModelListCtrlAdaptor*>(this);
    return pThis->IsUndeletedInSelection();
}
...
bool DataModelListCtrlAdaptor::DoAdd()
{
    wxDataModelBase* db = m_listCtrl->GetModel();
    bool ok = db->AddNew();

    if (ok)
    {
        m_listCtrl->Fill();
        m_listCtrl->SelectNone();
        m_listCtrl->SelectRow(m_listCtrl->GetItemCount()-1);
        m_listCtrl->Edit();
    }
    return ok;
}

bool DataModelListCtrlAdaptor::IsUndeletedInSelection()
{
    bool ok = false;
    ...
    for (wxArrayInt::const_iterator it = selections.begin(); (!ok) && (it != selections.end()); it++)
        ok = ok || (m_listCtrl->IsRecordOk(*it) && !IsRecordDeleted(*it));
    return ok;
}

bool DataModelListCtrlAdaptor::IsDeletedInSelection()
{
    bool ok = false;
...
bool DataModelListCtrlAdaptor::IsRecordDeleted(unsigned int row)
{
    return m_listCtrl->GetModel()->IsRowDeleted(row);
}

bool DataModelListCtrlAdaptor::DeleteRecord(unsigned int row, bool bDelete)
{
    return m_listCtrl->GetModel()->DeleteRow(row, bDelete);
}
