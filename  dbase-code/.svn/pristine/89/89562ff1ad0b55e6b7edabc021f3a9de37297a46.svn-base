...
int CDbaseFile::Create(const TCHAR* filename, const CStringArray& strFieldArray)
{
   USES_CONVERSION;
    ...
    vector.resize(strFieldArray.GetSize());
    for (size_t x = 0; x < vector.size(); x++)
	{
      DBF_FIELD_INFO* field = &vector[x];
    ...
    parm.version = 0;
    parm.api = NULL;
    parm.charconv = dbf_charconv_compatible;
    parm.tablename = NULL;
    parm.array = &vector[0];
    parm.array_count = vector.size();
    DBF_HANDLE handle = ::dbf_create(T2CA(filename), &parm);
    if (handle)
    {
        Attach(handle);
