...
public:
    ...
    int Open(const TCHAR* filePath, dbf_editmode editmode = dbf_editmode_editable);
    int Open(const TCHAR* filePath, const DBF_OPEN&);
    int Create(const TCHAR* filePath, const CStringArray& strFieldArray);
...
public:
    CString GetMemoFileName(const TCHAR* filePath = NULL) const;
    int OpenMemoFile(const TCHAR* filePath = NULL);
    //int CreateMemoFile();

    size_t GetMemoFieldLength(size_t field);
    int GetMemoField(size_t field, CString* buf, size_t buf_len);
