...
wxIMPLEMENT_DYNAMIC_CLASS(DBFFrame, wxDocMDIChildFrame);
...
wxBEGIN_EVENT_TABLE(DBFFrame, wxDocMDIChildFrame)
wxEND_EVENT_TABLE()
...
bool DBFFrame::Create(wxMDIParentFrame* parent, wxView* view, wxRecentFileList* mru)
{
    ...
    if (ok)
    {
        ...
        Show(false); // hidden is wx trunk default
    #endif
        m_mru = mru;
...
/*static*/ const wxAcceleratorVector& DBFFrame::GetAccelerator()
{
    ...
    if (array.empty())
    {
        ...
        array.push_back(wxAcceleratorEntry(wxACCEL_NORMAL, WXK_NUMPAD_ADD, wxID_ADD));
        ...
        array.push_back(wxAcceleratorEntry(wxACCEL_CTRL, 'X', XRCID("file_export")));
