...
void CVecPnN::WriteIni(CStdioFile& file)
{
	...
	for (unsigned i = 0; i < m_vecPnN.size(); i++)
	{
		...
		if (int(p.m_NAYZ) == 0)
			continue;
		str=CString::Format(_T("№узла_начало=%g\n"), p.m_NAYZ);
		file.WriteString(str);
		str=CString::Format(_T("№узла_конец=%g\n"), p.m_KOYZ);
		file.WriteString(str);
		str=CString::Format(_T("X=%g\nY=%g\nZ=%g\n"), p.m_OSIX, p.m_OSIY, p.m_OSIZ);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Диаметр=%g\nМатериал="), p.m_DIAM);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str = p.m_NAMA;
		file.WriteString(str + _T("\n"));
		str=CString::Format(_T("Толщина_ном=%g\nПрибавка=%g\n"), p.m_NTOS, p.m_RTOS);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Прибавка_кор=%g\nР=%g\n"), 0.0f, p.m_RADA);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Р_исп=%g\nТ=%g\n"), p.m_DABI, p.m_RATE);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Вес_трубы=%g\nВес_изоляции=%g\nВес_продукта=%g\n"), p.m_VETR, p.m_VEIZ, p.m_VEPR);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("К_изгиб=%g\nК_давление=%g\n"), p.m_KOPR, p.m_KOPE);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		bool bPodzem = fabs(p.m_NAGV + 1) < 1e-6;
		if (!bPodzem)
		{
			str=CString::Format(_T("Дополнит_весов_нагр=%g\nМ_доп_X=%g\nМ_доп_Y=%g\nМ_доп_Z=%g\n"),
		...
		else
		{
			...
			str=CString::Format(_T("Высота_воды=%g\nТолщина_кожуха=%g\nШирина_траншеи=%g\n"), 0.0f, set.m_IZTO, p.m_SHTR);
			set.Close();
			str.Replace(_T(","),_T("."));
			file.WriteString(str);
			...
			str=CString::Format(_T("Грунт_основание=0%d\nГрунт_верх=0%d\nГрунт_бок=0%d\n"), nDown, nUp, nSide);
			file.WriteString(str);
			str=CString::Format(_T("Тип_изоляции=%d\nNm=%g\n"), 3, 0.67f);
			str.Replace(_T(","),_T("."));
			file.WriteString(str);
			str=CString::Format(_T("Тип_подушек=%d\nПросадка=%g\n"), nPodush, 0.0f);
			str.Replace(_T(","),_T("."));
			file.WriteString(str);
		}
	}
	...
	for (unsigned i = 0; i < m_vecPnN.size(); i++)
	{
		...
		str=CString::Format(_T("№узла=%g\n"), p.m_KOYZ);
		file.WriteString(str);
		str=CString::Format(_T("Тип=%d\nРадиус=%g\n"), nType, p.m_RAOT * 1000);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Вес=%g\nМатериал="), p.m_VESA);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str = p.m_MARI;
		file.WriteString(str + _T("\n"));
		...
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("К_овал=%g\nФланцы=%d\n"), 0.0f, 0);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("К_давление=%g\nСкосы=%d\n"), 1.0f, 3);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
	}
	bFirst = true;
	for (unsigned i = 0; i < m_vecPnN.size(); i++)
	{
		...
		CString str1;
		str1 = p.m_MARI;
		str=CString::Format(_T("Материал=%s\n"), LPCTSTR(str1));
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Толщина_маг_ном=%g\nПрибавка_маг=%g\n"), p.m_NOTO, p.m_RATO);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Толщина_штуц_ном=%g\nПрибавка_штуц=%g\n"), p.m_VEYS, p.m_DIGI);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Высота_штуц=%g\n"), p.m_SILX);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Толщина_накл=%g\nШирина_накл=%g\n"), p.m_DEFZ, p.m_RASG);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
	}
	bFirst = true;
	...
	for (unsigned i = 0; i < m_vecPnN.size(); i++)
	{
		...
		str=CString::Format(_T("№узла=%g\n"), p.m_KOYZ);
		file.WriteString(str);
		str=CString::Format(_T("Податливость=%g\n"), p.m_KOTR);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
	}
	bFirst = true;
	for (unsigned i = 0; i < m_vecPnN.size(); i++)
	{
		...
		str=CString::Format(_T("Тип=%d\n"), bPodzem ? 0 : 1);
		file.WriteString(str);
		str=CString::Format(_T("Площадь_эффект=%g\nПодатливость=%g\nОсевой_ход=%g\n"), p.m_RAOT, p.m_KOTR, p.m_DIGI);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
	}
	bFirst = true;
	...
	for (unsigned i = 0; i < m_vecPnN.size(); i++)
	{
		...
		str=CString::Format(_T("№узла=%g\n"), p.m_KOYZ);
		file.WriteString(str);
		str=CString::Format(_T("К_трения=%g\n"), p.m_KOTR);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
	}
	bFirst = true;
	...
	for (unsigned i = 0; i < m_vecPnN.size(); i++)
	{
		...
		str=CString::Format(_T("№узла=%g\n"), p.m_KOYZ);
		file.WriteString(str);
		str=CString::Format(_T("Число_тяг=%g\nИзменение_нагр=%g\n"), p.m_SEOP, p.m_NOTO);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("К_запаса_нагр=%g\nПоддержив_усилие=%g\nПодатливость=%g\n"),
	...
	for (unsigned i = 0; i < m_vecPnN.size(); i++)
	{
		...
		str=CString::Format(_T("№узла=%g\n"), p.m_KOYZ);
		file.WriteString(str);
		str=CString::Format(_T("Длина_тяги=%g\n"), p.m_DIGI);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
	}
	bFirst = true;
	for (unsigned i = 0; i < m_vecPnN.size(); i++)
	{
		...
		file.WriteString(str);
		if (p.m_TIDE == "рс")
			str=CString::Format(_T("Растяжение=%g\n"), p.m_RASG);
		else
			str=CString::Format(_T("Сжатие=%g\n"), p.m_RASG);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
	}
	...
	for (unsigned i = 0; i < m_vecPnN.size(); i++)
	{
		...
		str=CString::Format(_T("№узла=%g\n"), p.m_KOYZ);
		file.WriteString(str);
		str=CString::Format(_T("Лин=%d\n"), p.m_PELI == "л" ? 1 : 0);
		file.WriteString(str);
		str=CString::Format(_T("Угл=%d\n"), p.m_PEYG == "у" ? 1 : 0);
		file.WriteString(str);
		str=CString::Format(_T("Сила_X=%g\nСила_Y=%g\nСила_Z=%g\n"), p.m_MNEA == "тр" ? 0.0f : p.m_SILX, p.m_SILY, p.m_SILZ);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Момент_X=%g\nМомент_Y=%g\nМомент_Z=%g\n"), p.m_MOMX, p.m_MOMY, p.m_MOMZ);
		str.Replace(_T(","),_T("."));
		file.WriteString(str);
		str=CString::Format(_T("Сила_весовая=%g\nМомент_вес_X=%g\nМомент_вес_Y=%g\n"),
