...
public:
	...
	{
		return m_nMaxRow - m_nMinRow + 1;
	}

	int GetColSpan() const
	{
		return m_nMaxCol - m_nMinCol + 1;
	}

	void operator=(const CCellRange& rhs);
	int operator==(const CCellRange& rhs) const;
	int operator!=(const CCellRange& rhs) const;
...
inline int CCellRange::operator==(const CCellRange& rhs) const
{
