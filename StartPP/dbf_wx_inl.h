...
{
   wxASSERT(!IsOpen());
   bool ok = base::Open(fileName.GetFullPath().mb_str(), editmode, fileName.GetName().mb_str());
   return ok;
}

inline bool wxDBase::Open(const wxFileName& fileName, const DBF_OPEN& parm)
{
   wxASSERT(!IsOpen());
   bool ok = base::Open(fileName.GetFullPath().mb_str(), parm);
   return ok;
}

inline bool wxDBase::Create(const wxFileName& fileName, const DBaseCreateParm& parm)
{
   wxASSERT(!IsOpen());
   bool ok = base::Create(fileName.GetFullPath().mb_str(), parm);
   return ok;
}

inline bool wxDBase::Create(void* stream, const DBaseCreateParm& parm, void* memoStream)
{
   wxASSERT(!IsOpen());
...
/*
inline bool wxDBase::Create(wxOutputStream* stream, const CreateParm& parm, void* memoStream)
{
   wxASSERT(!IsOpen());
   bool ok = base::Create(stream, parm, memoStream);
   return ok;
