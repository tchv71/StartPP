...
wxGLContext::wxGLContext(wxGLCanvas* win, const wxGLContext* other, const wxGLContextAttrs* ctxAttrs)
    : m_glContext(NULL)
{
...
bool wxGLContext::SetCurrent(const wxGLCanvas& win) const
{
    ...
    const_cast<wxGLCanvas&>(win).attach_buffers();
}



wxGLCanvasGdk::wxGLCanvasGdk() :
    have_buffers(FALSE),
    frame_buffer(0),
    render_buffer(0),
...
void wxGLCanvasGdk::ensure_buffers ()
{
    if (have_buffers)
        return;
...
void wxGLCanvasGdk::allocate_buffers ()
{
...
void wxGLCanvasGdk::attach_buffers ()
{
    GdkGLContext* context = gdk_gl_context_get_current();
    if(!context)
        return;
...
bool wxGLCanvasGdk::SwapBuffers()
{
    GdkGLContext* context = gdk_gl_context_get_current();
    if(!context)
        return false;
    ...
    {
        wxGLCanvas *pChild = dynamic_cast<wxGLCanvas*>(this);
...
bool wxGLCanvasGdk::InitVisual(const wxGLAttributes& dispAttrs, GdkWindow*)
{
    return true;
}
